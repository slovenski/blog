---
title: Containerize your Laravel application with Docker
...
In the [previous articles](https://gibransvarga.com/highly-available-applications-docker-swarm/) we have managed architecture for High Availability applications with Docker using GlusterFS Replicated Volume. Now it’s time to get into more specifics by containerize a Laravel application and then test if this setup is resilient to VM failure.
...
You can repeat this test to MySQL and Nginx services. And make sure docker swarm has scheduled the services on another worker machine and the application will continue to work without any data loss. In a production scenario, the swarm worker nodes can be fronted by a load balancer or a service discovery which directs the Laravel requests to the node running the container.

To return the drained node to an active state, run `docker node update --availability active <NODE>` command like follows:
